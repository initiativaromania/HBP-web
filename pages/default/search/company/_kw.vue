<template>
  <b-container fluid>
    ...
    <b-row><b-col>
      <b-table ref="companyTable" striped responsive stacked="md" small hover :items="getResults" :per-page="pageSize" :current-page="page"
      :fields="fields">
...
export default {
  ...
  //   let { data } = await app.$axios.get(`search/company/${params.kw}`)
  ...
  methods: {
    doSearch: function () {
      this.$router.replace({name: 'default-search-company-kw', params: {kw: this.kw}})
    },
    getResults: function (ctx) {
      let req = this.$axios.get(`/search/company/${this.kw}?page=${ctx.currentPage}` +
