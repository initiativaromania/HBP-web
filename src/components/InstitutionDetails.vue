    ...
    <div>
      <b-tabs card>
        ...
        <b-tab :title="'Achiziții directe ('+ details.contract_count +')'">
          <b-row>
            <b-col>
              <b-pagination :total-rows="details.contractsLength" :per-page="contractPageSize" v-model="contractPage" size="sm" :limit="10"/>
            </b-col>
            <b-col sm="1">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td class="text-center" colspan="2">
                        <b-btn size="sm" :to="{name: 'CompanyDetails', params: {id: currContract.CompanieId}}">{{currContract.NumeCompanie}}</b-btn>
                      </td>
                    </tr>
                  </tbody>
                </table>
              ...
              <b-container fluid v-if="currTender">
                <table class="table table-striped table-bordered" width="100%">
                  <tbody>
                    ...
                    <tr>
                      <td class="text-center" colspan="2">
                        <b-btn size="sm" :to="{name: 'CompanyDetails', params: {id: currTender.CompanieId}}" >{{currTender.NumeCompanie}}</b-btn>
                      </td>
                    </tr>
                  </tbody>
                </table>
...
function updateStats (vm, {details, hist, cpv}) {
  vm.details = details
  vm.volume_pie = [
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    Vue.axios.get('institution_stats/' + to.params.id).then(({data}) => {
      next(vm => updateStats(vm, data))
    })
  },
