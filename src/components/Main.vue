<template>
    <div id="gmapParent">
      <div id="gmap" />
      <div id="popup">
        <b-link :to="{name: 'InstitutionDetails', params: {id: selected.id}}" v-if="selected">
...
import definePopupClass from '@/common/gmapsPopup'
...
const pinIcon = {url: '/static/pin.svg', scaledSize: {width: 28, height: 40}, anchor: {x: 14, y: 40}}
const groupIcon = {url: '/static/cluster.svg', scaledSize: {width: 40, height: 40}, anchor: {x: 20, y: 20}}
  ...
  methods: {
      ...
      this.axios.get(`institutions?cluster=true&n=${ne.lat()}&s=${sw.lat()}&e=${ne.lng()}&w=${sw.lng()}&zoom=${zoom}&size=${mapSize}`).then(response => {
        ...
        for (let item of response.data) {
          ...
          if (item.count && item.count > 1) {
            marker = new this.map_api.Marker({
    ...
    doSearch: function () {
      if ('company' in this.params) {
        this.$router.push({name: 'SearchCompany', params: {kw: this.params.company}})
      } else if ('reg_no' in this.params) {
        this.axios.get(`../api/InstitutionByCUI/${this.params.reg_no}`).then(response => {
          if (!response.data.length) {
            this.axios.get(`../api/CompanyByCUI/${this.params.reg_no}`).then(response => {
              if (!response.data.length) {
                ...
                this.$router.push({name: 'CompanyDetails', params: {type: 'AD', id: response.data[0].CompanieId}})
              }
            })
          } else {
            this.$router.push({name: 'InstitutionDetails', params: {id: response.data[0].InstitutiePublicaId}})
          }
        })
      } else if ('institution' in this.params) {
        this.$router.push({name: 'SearchInstitution', params: {kw: this.params.institution}})
      } else if ('keywords' in this.params) {
        this.$router.push({name: 'SearchContracts', params: {kw: this.params.keywords}})
      }
    }
  },
  mounted () {
    require('google-maps-api')(process.env.GMAP_API_KEY)().then(maps => {
      this.map_api = maps
      this.map = new maps.Map(document.getElementById('gmap'), mapOptions)
      this.map.addListener('bounds_changed', _.debounce(this.getPins, 200))
